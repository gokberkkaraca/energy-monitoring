...
/**
 * Created by novak on 12/17/2017.
 */
...
public class WearListenerService extends WearableListenerService{
    ...
    public static final String DATAMAP_STRING_USER = "DATAMAP_STRING_USER";
    ...
    public void onCreate() {
        ...
        if (mGoogleApiClient.isConnected()) Log.d("Degug","connected to wear");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        switch(action) {
            ...
            case ACTION_SEND_DATAMAP:
                ...
                break;
            case ACTION_SEND_USER:
                Log.d(TAG,"Received user data");
                PutDataMapRequest putDataMapRequestUser = PutDataMapRequest.create(BuildConfig.user_path);
                putDataMapRequestUser.getDataMap().putString(BuildConfig.user_key, intent.getStringExtra(DATAMAP_STRING_USER));
                sendPutDataMapRequest(putDataMapRequestUser);
                break;
            default:
            ...
            if (event.getType() == DataEvent.TYPE_CHANGED) {
                ...
                switch (uri.getPath()) {
                    ...
                    case BuildConfig.location_path:
                        ...
                        intent = new Intent(MapsFragment.ACTION_SEND_LOCATION);
                        intent.putExtra(MapsFragment.INTEGER_KEY_LOCATION, integerLocation);
                        intent.putExtra(MapsFragment.FLOAT_ARRAY_LOCATION, locationArray);
                        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
                        break;
                    ...
                    case BuildConfig.heart_rate_path:
                        ...
                        intent = new Intent(DeviceControlActivity.ACTION_SEND_HEART_RATE);
                        intent.putExtra(DeviceControlActivity.INT_HEART_RATE, heartRate);
                        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
                        break;
*/
                    case BuildConfig.user_path:
                        String userData = dataMapItem.getDataMap().getString(BuildConfig.user_key);
                        Log.i(TAG, "Got user Data: " + userData);
                        intent = new Intent(MainActivity.ACTION_SEND_USER); // which MainActivity?
                        intent.putExtra(MainActivity.STRING_USER, userData);
                        LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
                        break;
                    default:
